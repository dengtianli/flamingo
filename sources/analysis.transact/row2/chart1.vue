...
<script>
  ...
  export default {
    ...
    mounted() {
      const vm = this;
      vm.managedItems(vm.year, vm.organ)
    },
    watch: {
      year: function(year, oldVal) {
        console.log(year)
        this.managedItems(year, this.organ)
      },
      organ: function(organ, oldVal) {
        console.log(organ)
        this.managedItems(this.year, organ)
      }
    },
    methods: {
      showChart(instantCount, promiseCount, totalCount) {
        echarts.dispose(document.querySelector("#row2chart1"));
        const chart = echarts.init(document.querySelector("#row2chart1"));
        chart.setOption({
          tooltip: {
            trigger: 'axis',
            axisPointer: {
      ...
      managedItems(year, organ) {
        const vm = this;
        Http.fetch({
          method: "get",
          url: master + "/analysis/projects/projects_finish",
          params: {
            year: year,
            organ: organ
          }
